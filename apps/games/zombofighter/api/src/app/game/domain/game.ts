...
export class Game implements GameInterface {
    ...
    public getSnapshot(): GameSnapshot {
        return {
            id: this.id,
            players: {
                [this.fighter.player.id]: Game.createSnapshotByFighter(
                    this.fighter
                ),
                [this.enemy.player.id]: Game.createSnapshotByFighter(
                    this.enemy
                ),
            },
    ...
    private createNextRound() {
        ...
        console.log(
            'push round',
            round.fighter.getActions(),
            round.enemy.getActions()
        )
    ...
    private createSnapshotByRounds(): RoundSnapshot[] {
        ...
        return this.rounds.map((round) => {
            ...
            if (fighterActions) {
                result[round.fighter.player.id] = Game.actionsToSnap(
                    fighterActions.damage,
                    fighterActions.protection
                )
            }

            if (enemyActions) {
                result[round.enemy.player.id] = Game.actionsToSnap(
                    enemyActions.damage,
                    enemyActions.protection
                )
            }
