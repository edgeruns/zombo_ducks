...
import user, { UserSkins } from '@apps/games/zombofighter/client/features/shared/user'
...
export const isActionSended = (state: FeatureState) => state.singleGame.isActionSended

export const getPlayerAttacks = (state: FeatureState) => state.singleGame.selectedAttacks
export const getPlayerDefences = (state: FeatureState) => state.singleGame.selectedDefences
...
export const getEnemyAttacks = createSelector(
    [getEnemyAction],
    (action) => {
        if (!action) {
            return [0, 0, 0]
...
export const getEnemyDefences = createSelector(
    [getEnemyAction],
    (action) => {
        if (!action) {
            return [0, 0, 0]
...
export const isQuitPopupVisible = (state: FeatureState) => state.singleGame.isQuitPopupVisible
