...
export const sendAction = createAsyncThunk<SendActionPayload, SendAction>(
    'sendAction',
    async (args) => {
...
export const receiveAction = createAsyncThunk<
    ReceiveActionPayload,
    ReceiveAction
>('receiveAction', async (args) => {
    console.log(`Receive action ${args.type}:`, args.data)
    return args
})

export const getPlayer = createAsyncThunk<User, void>(
    'getPlayer',
    async (_) => {
        return {
            id: 1,
...
export const startSearch = createAsyncThunk<void, void, { state: AppState }>(
    'startSearch',
...
export const attack = createAsyncThunk<
    ...
    void,
    { state: AppState }
>('attack', async (_, { getState, dispatch }) => {
    const state = getState()
    const player = selectors.getPlayer(state)
...
export const quitGame = createAsyncThunk<
    ...
    void,
    { state: AppState }
>('quitGame', async (_, { dispatch, getState }) => {
    const state = getState()
