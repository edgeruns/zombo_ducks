...
export const isQuitPopupOpened = (appState: AppState) =>
    appState.bar.quitPopupOpened
...
export const getPlayerHealth = createSelector([getRoundPlayer], (player) => {
...
export const getPlayerStatus = createSelector(
    ...
    (damage, gameResult, isRoundFinish, isGameFinish) => {
        if (isGameFinish && gameResult) {
            return gameResult.type === GameResultType.Victory
                ? UserStatus.Victory
                : UserStatus.Lose
        }
...
export const isGameVictory = createSelector([getGameResult], (result) => {
    return result?.type === GameResultType.Victory
})

export const isGameLose = createSelector([getGameResult], (result) => {
